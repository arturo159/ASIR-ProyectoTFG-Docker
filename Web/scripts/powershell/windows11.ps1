...
if (-not $exists) {
    ...
    if ($kvmCheck -match '/dev/kvm') {
        ...
        docker run -d --name $containerName -p 8006:8006 --cap-add NET_ADMIN --device=/dev/kvm -e VERSION=win11 -e RAM_SIZE=4G -e CPU_CORES=2 -e DISK_SIZE=64G -e KVM=$envKvm -v windows_disk:/storage dockurr/windows
    } else {
        ...
        docker run -d --name $containerName -p 8006:8006 --cap-add NET_ADMIN -e VERSION=win11 -e RAM_SIZE=4G -e CPU_CORES=2 -e DISK_SIZE=64G -e KVM=$envKvm -v windows_disk:/storage dockurr/windows
    }
}
